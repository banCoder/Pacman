...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        Initializes a set to store visited corners."""
        return (self.startingPosition, set())

    def isGoalState(self, state):
        """Returns whether this search state is a goal state of the problem.

        It's a goal if it contains all corners."""
        return len(state[1]) == len(self.corners)

    def getSuccessors(self, state):
        """Returns successor states, the actions they require, and a cost of 1.

        State stores current position and set of visited corners.
        Adds corner to visited for successors."""
        successors = []
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x,y = state[0]
            ...
            if hitsWall == False:
                ...
                if next_node in self.corners:
                    nextVisitedCorners.add(next_node)
                successors.append(((next_node, nextVisitedCorners), action, 1))
        self._expanded += 1 # DO NOT CHANGE
        return successors
